...
@section('content')
    <section class="relative py-20" style="background-image: url('{{ asset('storage/' . $thisCategory->image->path) }}'); background-size: cover;">
        <div class="absolute inset-0 bg-gradient-to-b from-gray-800 to-black opacity-90"></div>
        <div class="container  mx-auto mt-12 text-white relative md:ml-12 z-10">
          <div class="ml-4">
              <h2 class="text-3xl font-bold mb-4 md:mb-6">LEARNING PATH</h2>
              <h1 class="text-5xl font-bold mb-6">{{$thisCategory->name}}</h1>
              <p class="text-lg text-gray-200 mb-8 max-w-xl">{{$thisCategory->description}}</p>
          </div>
            <div class="flex flex-col ml-4 md:flex-row justify-start items-start mb-8 space-y-4 md:space-y-0 md:space-x-8">
                ...
                @foreach ($lessons as $lesson)
                    <div class="lg:flex border-2 rounded-lg overflow-hidden hover:shadow-xl ">
                        <img class="object-cover w-full h-56 rounded-lg lg:w-64"
                             ...
                             alt="Lesson Image">
                        <div class="flex flex-col justify-between py-6 m-2 lg:mx-6">
